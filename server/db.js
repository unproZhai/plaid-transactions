...
async function listTables() {
  try {
    ...
    console.log("Tables in the database:", tableNames.map(table => table.name));
  } catch (error) {
    console.error("Error fetching table names:", error);
  }
}
async function selectEverythingFromTable(tableName) {
  try {
  ...
  const tables = ['users', 'items', 'accounts', 'transactions'];
  for (const tableName of tables) {
    await selectEverythingFromTable(tableName);
  }
}


dbWrapper.open({ filename: databaseFile, driver: sqlite3.Database }).then(async (db) => {
  try {
    await listTables(); 
    await queryAllTables().catch(console.error);
  } catch (error) {
